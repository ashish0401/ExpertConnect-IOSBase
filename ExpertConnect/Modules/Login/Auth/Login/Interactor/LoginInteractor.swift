...
final class LoginInteractor: LoginInteractorInputProtocol {
    ...
    func authenticateUserWithFacebook(model: LoginWithFacebookDomainModel) {
        ...
        if (userAlreadyLoggedIn)! {
            ...
            APIDataManager?.authenticateUserWithFacebook(model: model, callback: { (result) in
                switch result {
                case .Failure(let error):
                    self.presenter?.onUserLoginWithFacebookFailed(error: error)
                case .Success(let data as LoginOutputDomainModel):
                    do {
                        try UserService.sharedInstance.createUser(username: data.firstName, token: "Example-Token")
                        self.presenter?.onUserLoginWithFacebookSucceeded(data: data)
                    } catch {
                        self.presenter?.onUserLoginWithFacebookFailed(error: EApiErrorType.InternalError)
                    }
                default:
                    break
                }
