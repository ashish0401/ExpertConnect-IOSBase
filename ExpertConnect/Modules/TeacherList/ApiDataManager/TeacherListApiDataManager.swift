...
final class TeacherListApiDataManager: TeacherListProtocols {
    ...
    func getTeacherList(data:TeacherListDomainModel,  callback: @escaping (ECallbackResultType) -> Void) {
        do {
            ...
            let url: String = try api.constructApiEndpoint(base: "http://182.72.44.11/expert_connect", params: "teacher_listing.php")
            let headers = try api.constructHeader(withCsrfToken: true, cookieDictionary: nil)
            let parameters = ["user_id" : data.userId, "category_id" : data.categoryId, "sub_category_id" : data.subCategoryId, "level" : data.level] as [String : Any]
