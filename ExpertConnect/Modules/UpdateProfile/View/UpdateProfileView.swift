...
class UpdateProfileView: UIViewController, UITextFieldDelegate, UIScrollViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, CLLocationManagerDelegate {
    ...
    func askForSaveIfUserIsSure() {
        let message = "Are you sure you want to leave? Changes will be discarded".localized(in: "UpdateProfileView")
        let cancel = "Cancel".localized(in: "UpdateProfileView")
        let ok = "Ok".localized(in: "UpdateProfileView")
        
        let saveAlert = UIAlertController(title: "Exit", message: message, preferredStyle: .alert)
