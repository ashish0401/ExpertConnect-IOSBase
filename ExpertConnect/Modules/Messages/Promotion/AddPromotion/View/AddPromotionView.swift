    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        if textField == self.dateTextfield {
            ...
            if (self.mainCategoryTextfield.text?.characters.count)! == 0 {
                ...
                self.basePriceTextfield.text = "AU$"
    ...
    func inputAccessoryViewDidFinishForDoneButton(button : UIButton) {
        ...
        if button.tag == 101 {
            ...
            if self.pickerviewExpertDetails.tag == 102 {
                ...
                self.basePriceTextfield.text = "AU$ \(basePrice)"
...
//          let substring = string?.substring(to: index!)
            ...
            if(Int(string!)! >= Int(basePriceStrSubstring!)!) {
                let message = "Discounted Price Should Not Be Greater Than The Base Price".localized(in: "AddPromotionView")
                self.displayErrorMessage(message: message)
                return
            }
        }
