...
@IBDesignable public class CosmosView: UIView {
    ...
    public func update() {
        ...
        var layers = CosmosLayers.createStarLayers(rating: rating, settings: settings)
        ...
        if let text = text {
            let textLayer = createTextLayer(text: text, layers: layers)
    ...
    private func createTextLayer(text: String, layers: [CALayer]) -> CALayer {
        let textLayer = CosmosLayerHelper.createTextLayer(text: text,
                                                          font: settings.textFont, color: settings.textColor)
        
        let starsSize = CosmosSize.calculateSizeToFitLayers(layers: layers)
        
        CosmosText.position(layer: textLayer, starsSize: starsSize, textMargin: settings.textMargin)
    ...
    private func updateSize(layers: [CALayer]) {
        viewSize = CosmosSize.calculateSizeToFitLayers(layers: layers)
    ...
    private func updateAccessibility() {
        CosmosAccessibility.update(view: self, rating: rating, text: text, settings: settings)
    }
    ...
    public override func accessibilityIncrement() {
        ...
        rating += CosmosAccessibility.accessibilityIncrement(rating: rating, settings: settings)
    ...
    public override func accessibilityDecrement() {
        ...
        rating -= CosmosAccessibility.accessibilityDecrement(rating: rating, settings: settings)
        didTouchCosmos?(rating)
        didFinishTouchingCosmos?(rating)
    }
    ...
    public override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touch = touches.first {
            ...
            onDidTouch(locationX: location, starsWidth: widthOfStars)
        }
    }
    ...
    func onDidTouch(locationX: CGFloat, starsWidth: CGFloat) {
        let calculatedTouchRating = CosmosTouch.touchRating(locationX: locationX, starsWidth: starsWidth,
                                                            settings: settings)
    ...
    var widthOfStars: CGFloat {
        if let sublayers = self.layer.sublayers , settings.totalStars <= sublayers.count {
            ...
            return CosmosSize.calculateSizeToFitLayers(layers: starLayers).width
        }
    ...
    override public func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        let oprimizedBounds = CosmosTouchTarget.optimize(bounds: bounds)
