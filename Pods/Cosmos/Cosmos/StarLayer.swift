...
struct StarLayer {
    ...
    static func create(starPoints: [CGPoint], size: Double,
        ...
        let containerLayer = createContainerLayer(size: size)
        let path = createStarPath(starPoints: starPoints, size: size, lineWidth: lineWidth)
        
        let shapeLayer = createShapeLayer(path: path.cgPath, lineWidth: lineWidth,
    ...
    static func createStarPath(starPoints: [CGPoint], size: Double,
        ...
        let points = scaleStar(starPoints: starPoints, factor: sizeWithoutLineWidth / 100,
