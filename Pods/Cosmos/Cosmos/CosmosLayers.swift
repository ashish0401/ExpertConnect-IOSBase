...
class CosmosLayers {
    ...
    class func createStarLayers(rating: Double, settings: CosmosSettings) -> [CALayer] {
        ...
        var ratingRemander = CosmosRating.numberOfFilledStars(rating: rating,
        ...
        for _ in (0..<settings.totalStars) {
            ...
            let starLayer = createCompositeStarLayer(starFillLevel: fillLevel, settings: settings)
            starLayers.append(starLayer)
            ratingRemander -= 1
        }
        
        positionStarLayers(layers: starLayers, starMargin: settings.starMargin)
        return starLayers
    }
    ...
    class func createCompositeStarLayer(starFillLevel: Double, settings: CosmosSettings) -> CALayer {
        ...
        if starFillLevel >= 1 {
            return createStarLayer(isFilled: true, settings: settings)
        }
        
        if starFillLevel == 0 {
            return createStarLayer(isFilled: false, settings: settings)
        }
        
        return createPartialStar(starFillLevel: starFillLevel, settings: settings)
    }
    ...
    class func createPartialStar(starFillLevel: Double, settings: CosmosSettings) -> CALayer {
        let filledStar = createStarLayer(isFilled: true, settings: settings)
        let emptyStar = createStarLayer(isFilled: false, settings: settings)
    ...
    private class func createStarLayer(isFilled: Bool, settings: CosmosSettings) -> CALayer {
        ...
        return StarLayer.create(starPoints: settings.starPoints,
