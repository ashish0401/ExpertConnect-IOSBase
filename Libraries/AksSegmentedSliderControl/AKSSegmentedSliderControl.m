...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
       ...
       // _shadowColor = [UIColor colorWithWhite:0.0 alpha:1];
        _radiusCircle = 0.75;
        _moveFinalIndex = 0;
        _currentIndex = 0;
        _touchEnabled = YES;

        _strokeColorForeground = [UIColor colorWithWhite:0 alpha:1.0];
        _strokeSizeForeground = 0.0;

        CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();

        NSArray *gradientColors = [NSArray arrayWithObjects:
                                   (id)[UIColor whiteColor].CGColor,
                                   (id)[UIColor colorWithWhite : 0.0 alpha : 1.000].CGColor, nil];
        ...
        UIImage *imageToUse = [UIImage imageNamed:@"slider_dot"];
        holderView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, imageToUse.size.width + 10, imageToUse.size.height + 10)];
...
- (void)drawRect:(CGRect)rect {
    ...
    [self fillRect:drawingRect withColor:[UIColor colorWithRed:242/255.0f green:90/255.0f blue:11/255.0f alpha:1.0f].CGColor onContext:_context];
