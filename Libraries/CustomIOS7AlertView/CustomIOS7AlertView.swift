...
class CustomIOS7AlertView: UIView {
    ...
    //        return "hello, world!"
    //    }
    internal func catchString(withString: String)
    {
        stringObject = withString as NSString
        NSLog("%@", stringObject)
    }
    internal func show() {
        show(nil)
    ...
    internal func close(_ completion: ((Bool) -> Void)?) {
        ...
        //        Float(string:String)
        let str = alertView.value(forKeyPath: "layer.transform.rotation.z") as! Float
        let startRotation = Float(str)
    ...
    fileprivate func setObservers() {
        //        NotificationCenter.default.addObserver(self, selector: #selector(CustomIOS7AlertView.deviceOrientationDidChange(_:)), name: NSNotification.Name.UIDeviceOrientationDidChange, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(CustomIOS7AlertView.keyboardWillShow(_:)), name:NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(CustomIOS7AlertView.keyboardWillHide(_:)), name:NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
    ...
    fileprivate func createAlertView() -> UIView {
        ...
        if stringObject == "AlertWithSlider"{
            ...
            self.setupExpertConnectAlertWithSlider(view)
            view.layer.cornerRadius = 3
            return view
        }
        if stringObject == "AlertForRequest/Accept/Reject"{
            let screenSize = self.calculateScreenSize()
            let dialogSize = self.calculateDialogSize()
            let view = UIView(frame: CGRect(
                x: (screenSize.width - dialogSize.width) / 2,
        ...
        let view = UIView(frame: CGRect(
            x: (screenSize.width - dialogSize.width) / 2,
            y: ((screenSize.height - dialogSize.height) / 2)-50,
            width: dialogSize.width,
            height: dialogSize.height+150
        ))
    ...
    fileprivate func addButtonsToView(_ container: UIView) {
        ...
        for buttonIndex in 0...(buttonTitles!.count - 1) {
            //let button = UIButton.withType(UIButtonType.custom) as UIButton
    ...
    // addButtonsToViewoneLineoneButton
    fileprivate func addButtonsToViewoneLineoneButton(_ container: UIView) {
        if buttonTitles == nil || buttonTitles?.count == 0 {
            return
        }
        let buttonWidth = container.bounds.size.width-60
        
        for buttonIndex in 0...(buttonTitles!.count-1) {
            
            let button:UIButton! = UIButton.init(type: UIButtonType.custom)
            //            var ypoint = CGFloat()
            //            ypoint = container.bounds.size.height-75
            if buttonIndex == 0{
                
                button.frame = CGRect(
                    x: CGFloat(30),
                    y: container.bounds.size.height-85,
                    width: buttonWidth,
                    height: buttonHeight
                )
            }
            if buttonIndex == 1 {
                button.frame = CGRect(
                    x: CGFloat(30),
                    y: container.bounds.size.height-140,
                    width: buttonWidth,
                    height: buttonHeight
                )
            }
            ...
            button.addTarget(self, action: #selector(CustomIOS7AlertView.buttonTouchUpInside(_:)), for: UIControlEvents.touchUpInside)
            //            var num = String()
            //            num = (buttonTitles?[buttonIndex])!
            //            button.setTitle(buttonTitles![buttonIndex], for: UIControlState())
            //            button.setTitle((buttonTitles?[buttonIndex])!, for: UIControlState())
            
            //            for element in buttonTitles! {
            //                print(element)
            //            }
    ...
    fileprivate func addSubmitButtonToView(_ container: UIView) {
        if buttonTitles == nil || buttonTitles?.count == 0 {
            return
        }
        ...
        for buttonIndex in 0...(buttonTitles!.count-1) {
            ...
            button.frame = CGRect(
                x: CGFloat(80),
                y: container.bounds.size.height-235,
    ...
    //        let startRotation : CGFloat = self.value(forKeyPath: "layer.transform.rotation.z") as Any as! CGFloat
    //
    ////        CGAffineTransform rotation;
    //
    ////                let startRotation = self.valueForKeyPath("layer.transform.rotation.z").floatValue
    //
    ////                let startRotation = Float(self.value(forKeyPath: "layer.transform.rotation.z") as! String)
    ////
    ////
    ////
    ////        let str = self.value(forKeyPath: "layer.transform.rotation.z") as! String
    ////        let startRotation = Float(str)
    //
    ...
    internal func keyboardWillShow(_ notification: Notification) {
        ...
        var keyboardSize = ((notification as NSNotification).userInfo![UIKeyboardFrameBeginUserInfoKey]! as AnyObject).cgRectValue.size
